...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    for batch_idx in range(bs):
        ...
        for box_idx in range(num_boxes):
            ...
            for mystage in range(num_stages):
                # print(mystage, '0000000000')
                if best_anchor in anchor_mask[mystage]:
                    # print(mystage, '0000000000')
                    x_offset = true_boxes[batch_idx, box_idx, 0] * grid_sizes[mystage][1]
                    y_offset = true_boxes[batch_idx, box_idx, 1] * grid_sizes[mystage][0]
                    # Grid Index
                    grid_col = np.floor(x_offset).astype('int32')
                    grid_row = np.floor(y_offset).astype('int32')
                    anchor_idx = anchor_mask[mystage].index(best_anchor)
                    class_idx = true_boxes[batch_idx, box_idx, 4].astype('int32')
                    # y_true[mystage][batch_idx, grid_row, grid_col, anchor_idx, 0] = x_offset - grid_col  # x
                    # y_true[mystage][batch_idx, grid_row, grid_col, anchor_idx, 1] = y_offset - grid_row  # y
                    # y_true[mystage][batch_idx, grid_row, grid_col, anchor_idx, :4] = true_boxes_abs[batch_idx, box_idx, :4] # abs xywh
                    # print(mystage, 'iiiiiiiii')
                    # print(y_true[mystage].shape)
                    # print(y_true[mystage][batch_idx, grid_row, grid_col, anchor_idx, :2].shape)
                    y_true[mystage][batch_idx, grid_row, grid_col, anchor_idx, :2] = true_boxes_xy[batch_idx, box_idx, :]  # abs xy
                    y_true[mystage][batch_idx, grid_row, grid_col, anchor_idx, 2:4] = true_boxes_wh[batch_idx, box_idx, :]  # abs wh
                    y_true[mystage][batch_idx, grid_row, grid_col, anchor_idx, 4] = 1  # confidence
                    y_true[mystage][batch_idx, grid_row, grid_col, anchor_idx, 5 + class_idx] = 1  # one-hot encoding
                    # smooth
