...
class Yolov4(object):
    ...
    def build_model(self, load_pretrained=True):
        ...
        # self.yolo_model.input.shape = (None, 416, 416, 3)
        #这里只是定义模型的输入输出的形状！！！！！
        ...
        self.training_model.compile(optimizer=tf.keras.optimizers.legacy.Adam(learning_rate=1e-4),
    ...
    def save_inference_model(self,epoch, logs):
        # # 将训练模型转换为推理模型
        # inference_model = models.Model(self.training_model.input, self.inference_model.output)
        # # 保存推理模型
        # inference_model.save(f'./saveing/inference_model_epoch_{epoch}.h5')
        # inference_model.save(f'./savemodel')
        self.yolo_model.save(f'./savemodel2/savemodel-epoch{epoch}')
        self.yolo_model.save(f'./H52/H5-epoch{epoch}/model.h5')


    def fit(self, train_data_gen, epochs, val_data_gen=None, initial_epoch=0, callbacks=None):
        save_inference_model_callback = tf.keras.callbacks.LambdaCallback(on_epoch_end=self.save_inference_model)
        ...
        self.training_model.fit(train_data_gen,
                                ...
                                epochs=epochs,
                                callbacks=[save_inference_model_callback],
